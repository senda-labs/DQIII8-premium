...
def main() -> None:
    ...
    parser.add_argument(
        "--head", type=int, default=HEAD_DEFAULT, help="Characters to keep from the start"
    )
    parser.add_argument(
        "--tail", type=int, default=TAIL_DEFAULT, help="Characters to keep from the end"
    )
    parser.add_argument(
        "--threshold", type=int, default=THRESHOLD, help="Minimum chars before truncation kicks in"
    )
